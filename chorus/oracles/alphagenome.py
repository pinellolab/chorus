...
class AlphaGenomeOracle(OracleBase):
    ...
    def _load_direct(self, weights: str) -> None:
        try:
            ...
            # AlphaGenome fetches its GCS reference tables (gencode GTF /
            # splice-site / polyA feathers) fresh on every load via urllib —
            # not cached, and the GCS endpoint intermittently fails mid-load:
            # SSL UNEXPECTED_EOF at connect time, and http.client.IncompleteRead
            # while reading the body of the large (~330 MB) gencode GTF feather.
            # Retry at both the urlopen (connect) and pd.read_feather
            # (download+parse) levels; the latter covers the IncompleteRead.
