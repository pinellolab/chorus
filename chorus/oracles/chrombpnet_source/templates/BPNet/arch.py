...
def motif_module(
    one_hot_input,
    filters=MOTIF_MODULE_PARAMS['filters'], 
    kernel_sizes=MOTIF_MODULE_PARAMS['kernel_sizes'], 
    padding=MOTIF_MODULE_PARAMS['padding'], 
    name_prefix=None):
...
def syntax_module(
    motif_module_output,
    num_dilation_layers=\
        SYNTAX_MODULE_PARAMS['num_dilation_layers'], 
    filters=SYNTAX_MODULE_PARAMS['filters'], 
    kernel_size=SYNTAX_MODULE_PARAMS['kernel_size'],
    padding=SYNTAX_MODULE_PARAMS['padding'], 
...
def profile_head(
    syntax_module_out, 
    filters=PROFILE_HEAD_PARAMS['filters'], 
    kernel_size=PROFILE_HEAD_PARAMS['kernel_size'], 
    padding=PROFILE_HEAD_PARAMS['padding'], 
    name_prefix=None):
...
def counts_head(
    syntax_module_out, name, units=COUNTS_HEAD_PARAMS['units'], 
    dropouts=COUNTS_HEAD_PARAMS['dropouts'],
    activations=COUNTS_HEAD_PARAMS['activations'],    
    name_prefix=None):
...
def load_params(params):
    ...
    counts_head_params = COUNTS_HEAD_PARAMS
    ...
    counts_bias_module_params = COUNTS_BIAS_MODULE_PARAMS
...
# Try to instanciate a model
if __name__ == "__main__":
    import json
    with open("./chorus/oracles/chrombpnet_source/input_data.json", "r") as data:
        tasks_raw = json.load(data)

    tasks = {int(k): v for k, v in tasks_raw.items()}
    model = BPNet(tasks, {}, name_prefix="main")
    model_json = model.to_json()
    
    with open("model.json", "w") as fopen:
        fopen.write(model_json)

    model.load_weights("./examples/BP000273.1_model.h5")
    ...
    print(input_len)
    x_seq = np.random.randint(0, 2, size=(1, input_len, 4)).astype("float")
    print(x_seq)

    inputs = [x_seq]

    # Model stores these internally
    output_profile_len = model.output_profile_len
    tracks_for_each_task = model.tracks_for_each_task  # cumulative tracks per task

    # Compute number of bias tracks per task
    # (same computation as BPNet)
    
    profile_pred, logcount_pred = model.predict(
        [inputs, np.zeros((1, output_profile_len, 2), dtype="float32"), np.zeros((1, 2), dtype="float32")],
    )

    print(profile_pred, logcount_pred)

    # Apply softmax to the profile and multiply by
    count_pred = np.exp(logcount_pred)
    plus = profile_pred[..., 0]
    minus = profile_pred[..., 1]
