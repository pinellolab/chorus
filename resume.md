...
| **#133** | `rerender_examples.py` is dead (points at `examples/applications`, renamed to `walkthroughs` in 340f30e). **Do not just fix the path** — I tried, and it rewrote 15 reports from MB-scale to 0.01 MB because `from_dict` drops the per-bin IGV arrays. A crash is safer than silent data loss. |
