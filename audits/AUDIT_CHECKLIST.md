...
- [ ] Same-machine back-to-back **in one process**: identical predictions. Verified bitwise for borzoi, cherimoya, chrombpnet, enformer, epinformerseq, legnet, sei. ⚠ **This proves very little on its own** — Enformer is bit-exact in-process and drifts up to 3.4% *between* processes, which is the path `regenerate_examples.py` takes. Treat an in-process pass as necessary, not sufficient; the cross-process table in `audits/2026-08-14_f8_localisation.md` is the real record. **P1**
- [ ] **AlphaGenome (JAX) measures bit-exact cross-process as of 2026-08-17** — `gate_end_to_end_determinism.py --oracle alphagenome`, 3 runs, `worst relative delta 0.000e+00` every time. This entry previously read "NOT deterministic run-to-run" on the strength of two consecutive calls differing on all 64 raw values (~0.1–0.4%); that predates `determinism.py` pinning `--xla_gpu_deterministic_ops=true`, which is the likely cause of the change. ⚠ **The gate must be run from `chorus-alphagenome`, not the base env** — its alphagenome path uses `use_environment=False`, so from `chorus` it dies with `No module named 'jax'` *after* argument parsing, which reads as a model failure rather than an env one. AlphaGenome is also the gate's default oracle, so the obvious invocation is the one that fails. **P1**
