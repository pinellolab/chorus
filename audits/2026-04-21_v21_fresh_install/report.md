# v21 fresh-install audit — 2026-04-21 (macOS arm64, data-cache purge)

> **Reconciliation note:** This audit was run **in parallel** with the
> main-branch v21 (`abfc7a3` — `audits/2026-04-21_v21_fresh_install_linux_cuda.md`).
> The two are complementary:
>
> - **This run (macOS arm64)** — data-cache purge only (`~/.chorus/backgrounds`,
>   `genomes/hg38.fa`), existing conda envs reused. 18 selenium screenshots,
>   10 probe logs.
> - **Main-branch v21 (Linux/CUDA)** — full fresh `mamba env create --prefix`
>   + isolated `HOME=$SCRATCH/home` so every home-default cache was empty.
>   Found + fixed 2 P1s (LegNet 230 bp drift + chrombpnet skip guard).
>
> This branch would have reverted the 2 P1 fixes since it was written
> before those landed on `chorus-applications`; only the artifacts are
> cherry-picked (screenshots + logs) — not the `test_integration.py` or
> notebook reverts or the deletion of the Linux/CUDA v21 report.
