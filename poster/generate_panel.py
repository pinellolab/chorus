...
# ---------------------------------------------------------------------------
# Paperclip integration (optional, best-effort)
# ---------------------------------------------------------------------------

def _paperclip_meta(pmc_id: str) -> dict | None:
    """If paperclip is installed, fetch the paper metadata for citation."""
    if not shutil.which("paperclip"):
        return None
    try:
        out = subprocess.run(
            ["paperclip", "lookup", "pmc", pmc_id],
            capture_output=True, text=True, timeout=15,
        )
        if out.returncode != 0 or "No documents found" in out.stdout:
            return None
        return {"raw": out.stdout, "found": True}
    except Exception:
        return None
...
def render(config: dict) -> str:
    ...
    if gt:
        ...
        # Optional paperclip lookup confirms the paper is indexed
        if gt.get("pmc_id") and _paperclip_meta(gt["pmc_id"]):
            citation_html += '<span style="margin-left:6px;color:#2F8F58">✓ in paperclip</span>'
        gt_html = f"""
    <div class="col">
