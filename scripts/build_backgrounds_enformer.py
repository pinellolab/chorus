...
parser.add_argument("--perbin-tail-k", type=int, default=19844,
                    help="Exact top/bottom K values kept per track for the perbin "
                         "layer, which cannot be retained whole (992,160 offered per track). Derived as "
...
def build_variant_backgrounds():
    ...
    if _suffix:
        # A position shard holds a PARTIAL reservoir for every track, so it
        # must ship raw samples: the CDF is built once, from the union, by
        # --part merge-shards. Writing a CDF per shard and pooling the grids
        # would only approximate the unsharded result.
        np.savez_compressed(
            interim_path,
